...
import { useLoginUserMutation } from "@/redux/services/nonAuthApi";
...
const Signin = () => {
  ...
  const { logInUser: loginInUserClient } = useAuth();
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    loginUser(values)
      .unwrap()
      .then((response) => {
        loginInUserClient(response.token);
        router.push("/panel");
      })
      .catch((err) => {
                ...
                <Form>
                  ...
                  <div className="flex flex-wrap items-center gap-10 md:justify-between xl:gap-15">
                    ...
                    <button
                      ...
                      className="hover:bg-blackho dark:bg-btndark dark:hover:bg-blackho inline-flex cursor-pointer items-center gap-2.5 rounded-full bg-black px-6 py-3 font-medium text-white duration-300 ease-in-out disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      {isLoading ? <MiniLoaderSpinner /> : "Giriş Yap"}
                      <svg
